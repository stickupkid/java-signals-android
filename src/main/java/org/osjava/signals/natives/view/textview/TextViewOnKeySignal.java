...
import org.osjava.signals.natives.view.textview.TextViewOnKeySignal.KeyEventType;
...
public class TextViewOnKeySignal extends
		NativeSignalImpl5<TextView, KeyEventType, Editable, Integer, KeyEvent> {
	 ...
	 */
	private TextViewOnKeySignal(TextView target, int inputType) {
		setTarget(target);
	...
	/**
	 * Create a newInstance of TextViewOnKeySignal
	 * 
	 * @return {@link TextViewOnKeySignal}
	 */
	@SuppressWarnings("unchecked")
	public static TextViewOnKeySignal newInstance() {
		return new TextViewOnKeySignal();
	}
	 ...
	 * @return {@link TextViewOnKeySignal}
	 */
	public static TextViewOnKeySignal newInstance(final TextView target, final int inputType) {
		return new TextViewOnKeySignal(target, inputType);
	}
	...
	private final class TargetListener implements KeyListener {
		...
		public boolean onKeyDown(View v, Editable text, int keyCode, KeyEvent event) {
			...
			try {
				dispatch((TextView) v, KeyEventType.DOWN, text, keyCode, event);
			} catch (Throwable t) {
		...
		public boolean onKeyUp(View v, Editable text, int keyCode, KeyEvent event) {
			...
			try {
				dispatch((TextView) v, KeyEventType.UP, text, keyCode, event);
			} catch (Throwable t) {
				Log.e(TAG_NAME, "Dispatch Error", t);
			}
